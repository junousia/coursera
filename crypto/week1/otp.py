...
def validKey(k, index):
    chars = 0.0
    total = 0.0
    for c in ciphers:
        if index < len(c):
            total += 1
            if re.search('[a-zA-Z."!?(),0-9]', chr(ord(k)^ord(c[index]))):
                chars += 1
    if chars/total >= 0.325:
        return True 
    else:
        return False
...
def main():
    for i in range(len(ciphers)):
        for j in range(i, len(ciphers)):
            if i == j:
                continue;
            for index, c in enumerate(strxor(ciphers[i], ciphers[j])):
                if re.match("[a-zA-Z]", c):
                    try:
                        #print "before {}".format(keys.get(index, []))
                        key1 = chr(ord(ciphers[i][index])^32)
                        key2 = chr(ord(ciphers[j][index])^32)
                        #if validKey(key1, index):
                        keys[index] = keys.get(index, []) + [key1]
                        #if validKey(key2, index):
                        keys[index] = keys.get(index, []) + [key2]
                        #print "after {}".format(keys.get(index, []))
                    except IndexError:
